...
- `highlightTodayWeekday`：默认关闭；开启后在表头将今天对应的星期文字标红，并在文字上方加一个 4px 红色圆点；其余六列保留透明占位点以保持基线一致；不加粗、不改变彩虹日期色彩节奏。
...
- README 最后提供“交流与反馈 / Community and feedback”群入口，复用博客固定二维码 URL；URL 查询参数由 `wechat-publisher-qr` skill 在每次换群码后刷新，用于规避 GitHub 与社区页面图片缓存。二维码更新只提交 README，不增加插件版本、不创建 Release。
...
- 已有核心功能：中英文本地化、每周起始日、非当日创建确认、可选 ISO 周号、彩虹日期、薄荷绿周末、可选今日星期高亮、键盘焦点与 aria-label。
